...
class JMapModelSitemap extends JMapModel implements IJMapModelSitemap, IJMapModelExportable {
	...
	private function runtimePreProcessing($query, $resultSourceObject) {
		...
		switch ($sqlqueryManaged->option) {
			...
			case 'com_jshopping':
				...
				if(!stristr($sqlqueryManaged->titlefield, $this->langTag) && stristr($sqlqueryManaged->table_maintable, 'jshopping')) {
					...
					// Now test if admin has created virtuemart 2 tables for the site user chosen language otherwise proceed with same of generated query
					$checkSafeQuery = "SHOW COLUMNS FROM " . $this->_db->nameQuote($sqlqueryManaged->table_maintable);
					$this->_db->setQuery($checkSafeQuery);
					$columnsArray = $this->_db->loadResultArray();
			...
			default:
				...
				if(!JMapLanguageMultilang::isEnabled() && stristr($query, '{langtag}')) {
					...
					$toReplaceString = "{langtag} OR " . $this->_db->nameQuote($sqlqueryManaged->table_maintable) . "." . 
														 $this->_db->nameQuote('language') . " != ''";
		...
		if($params->get('multilevel_categories', 0) && $this->hasCategorization($resultSourceObject)) {
			...
			if(is_object($manifestConfiguration) && isset($manifestConfiguration->recursion_type) && $manifestConfiguration->recursion_type == 'level') {
				$toReplaceString = "SELECT " . 
									$this->_db->nameQuote($manifestConfiguration->categories_table) . "." . 
									$this->_db->nameQuote($manifestConfiguration->level_field) . 
									" AS " . $this->_db->nameQuote('jsitemap_level') . ", ";
				...
				$toReplaceString = "SELECT " .
									$this->_db->nameQuote($manifestConfiguration->categories_table) . "." .
									$this->_db->nameQuote($manifestConfiguration->category_table_id_field) .
									" AS " . $this->_db->nameQuote('jsitemap_category_id') . ", ";
				$query = preg_replace("/SELECT/", $toReplaceString, $query, 1);
				$resultSourceObject->catRecursion = true;
				$resultSourceObject->recursionType = $manifestConfiguration->recursion_type;
				...
				$toReplaceString = "SELECT " .
									$this->_db->nameQuote($manifestConfiguration->item2category_table) . "." .
									$this->_db->nameQuote($manifestConfiguration->item2category_catid_field) .
									" AS " . $this->_db->nameQuote('jsitemap_category_id') . ", ";
				$query = preg_replace("/SELECT/", $toReplaceString, $query, 1);
				$resultSourceObject->catRecursion = true;
				$resultSourceObject->recursionType = $manifestConfiguration->recursion_type;
		...
		if(	!$hasJSitemapCategoryId
			...
			$toReplaceString = "SELECT " .
							   $this->_db->nameQuote($routeManifest->categories_table) . "." .
							   $this->_db->nameQuote($routeManifest->categories_table_id_field) .
							   " AS " . $this->_db->nameQuote('jsitemap_category_id') . ", ";
			$query = preg_replace("/SELECT/", $toReplaceString, $query, 1);
		}
	...
	private function runtimeRssPreProcessing($keyIndexTable, $query) {
		...
		$toReplaceString = "SELECT " .
						   $this->_db->nameQuote($keyIndexTable) . "." .
						   $this->_db->nameQuote($descField) .
						   " AS " . $this->_db->nameQuote('jsitemap_rss_desc') . ", ";
		$query = preg_replace("/SELECT/", $toReplaceString, $query, 1);
		...
		switch ($source->type) {
			...
			case 'content':
				...
				$contentQueryItems = "SELECT c.id, c.alias, c.language, c.publish_up, c.access, c.metakey, catspriorities.priority," .
									 ...
									 "\n c.catid AS catslug" .
									 $pageBreaksFullText .
									 "\n FROM " . $this->_db->nameQuote('#__content') . " AS c" .
	...
	private function getSources() {
		...
		$filterDataset = JRequest::getInt('dataset', null);
	...
	protected function fetchAutoInjectDefaultWhereFields($tableName, $manifestConfig, $source) {
		...
		$columnsQuery = "SHOW COLUMNS FROM " . $this->_db->nameQuote($tableName);
			...
			if($tableFields = $this->_db->loadResultArray()) {
				...
				if(is_array($tableFields) && count($tableFields)) {
					...
					if(in_array('published', $tableFields) && !(in_array('published', $excludeConditions))) {
						$whereConditions[] = $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('published') . " = " . $this->_db->quote(1);
					} elseif(in_array('state', $tableFields) && !(in_array('state', $excludeConditions))) { // State field supported fallback
						$whereConditions[] = $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('state') . " = " . $this->_db->quote(1);
					}
					...
					if(in_array('access', $tableFields) && !(in_array('access', $excludeConditions))) {
						...
						$whereConditions[] = $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('access') . " IN (" . implode(',', $this->accessLevel) . ")";
					}
				
					// Language field supported
					if(in_array('language', $tableFields) && !(in_array('language', $excludeConditions))) {
						$whereConditions[] =  " (" . $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('language') . " = " . $this->_db->quote('*') .
											  " OR " . $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('language') . " = " . $this->_db->quote('') .
											  " OR " . $this->_db->nameQuote($tableName) . "." . $this->_db->nameQuote('language')  . " = " . $this->_db->quote($this->langTag) . ")";
					}
	...
	protected function subQueriesPostProcessor($source, $manifest = null) {
		...
		// SELECT for catname
		$selectCatName = $this->_db->nameQuote($categoriesTable) . "." . $this->_db->nameQuote($asCategoryTableNameField) . " AS " . $this->_db->nameQuote('catname');
			...
			case 'multiadiacency':
				...
				if(isset($manifestConfiguration->additional_categories_table) && isset($manifestConfiguration->additional_categories_table_on_catid_field)) {
					if(strpos($manifestConfiguration->additional_categories_table, '$$$')) {
						...
						$checkTableQuery = "SELECT 1 FROM " . $this->_db->nameQuote($additionalCategoriesTable);
					...
					$selectCatName = $this->_db->nameQuote($additionalCategoriesTable) . "." . $this->_db->nameQuote($asCategoryTableNameField) . " AS " . $this->_db->nameQuote('catname');
					...
					if(isset($manifestConfiguration->additional_categories_table_on_fkcatid_field)) {
						...
						$categoriesJoinCondition = "\n INNER JOIN " . $this->_db->nameQuote($additionalCategoriesTable) .
												   "\n ON " . $this->_db->nameQuote($categoriesTable)  . "." .  $this->_db->nameQuote($additionalCategoriesTableOnCatidField) . " = " .
												   $this->_db->nameQuote($additionalCategoriesTable)  . "." .  $this->_db->nameQuote($additionalCategoriesTableOnFKCatidField);
					} else {
						$categoriesJoinCondition = "\n INNER JOIN " . $this->_db->nameQuote($additionalCategoriesTable) .
												   "\n ON " . $this->_db->nameQuote($categoriesTable)  . "." .  $this->_db->nameQuote($additionalCategoriesTableOnCatidField) . " = " .
												   $this->_db->nameQuote($additionalCategoriesTable)  . "." .  $this->_db->nameQuote($additionalCategoriesTableOnCatidField);
					}
				}
				...
				$query = "SELECT " .
						$this->_db->nameQuote($asCategoryParentIdField) . " AS " . $this->_db->nameQuote('parent') . "," .
						$this->_db->nameQuote($asCategoryChildIdField) . " AS " . $this->_db->nameQuote('child') .
						"\n FROM " . $this->_db->nameQuote($category2categoryTable) .
						$where;
		...
		$query = "SELECT DISTINCT " . 
				$this->_db->nameQuote($categoriesTable)  . "." .  $this->_db->nameQuote($asCategoryTableIdField) . " AS " . $this->_db->nameQuote('id') . "," .
				$selectCatName .
				"\n FROM " . $this->_db->nameQuote($categoriesTable) .
				$categoriesJoinCondition .
				$validCategoryCondition .
	...
	function __construct($config = array()) {
		...
		if(isset($config['jmap_module']) && $config['jmap_module']) {
			...
			$query->where($this->_db->nameQuote('id') . ' = ' . (int)$config['jmap_module']);
