...
switch ($targetViewName) {
		...
		if(isset($elm->jsitemap_category_id)) {
			...
			if(!$itemId) {
				...
				$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
						...
						. ' OR ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easydiscuss&view=forums&category_id='.$elm->jsitemap_category_id) . ') '
			...
			if(!$itemId) {
				...
				foreach ($parentCategories as $parentCat) {
					$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
							...
							. ' OR ' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easydiscuss&view=forums&category_id='.$parentCat) . ') '
							. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' )
							. $helperRouteClass::getLanguageQuery()
							. ' LIMIT 1';
					$db->setQuery( $query );
	...
	case 'categories':
		if(strpos($additionalQueryStringParams, 'layout=listings')) {
			...
			if(!$itemId && $isED4) {
				$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
						. 'WHERE (' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easydiscuss&view=forums&category_id='.$elm->category_id) . ') '
						. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' )
						. $helperRouteClass::getLanguageQuery()
						. ' LIMIT 1';
				$db->setQuery( $query );
			...
			if($itemId) {
				...
				foreach ($parentCategories as $parentCat) {
					...
					if(!$itemId && $isED4) {
						$query	= 'SELECT ' . $db->nameQuote('id') . ' FROM ' . $db->nameQuote( '#__menu' ) . ' '
								. 'WHERE (' . $db->nameQuote( 'link' ) . '=' . $db->Quote( 'index.php?option=com_easydiscuss&view=forums&category_id='.$parentCat) . ') '
								. 'AND ' . $db->nameQuote( 'published' ) . '=' . $db->Quote( '1' )
								. $helperRouteClass::getLanguageQuery()
								. ' LIMIT 1';
						$db->setQuery( $query );
