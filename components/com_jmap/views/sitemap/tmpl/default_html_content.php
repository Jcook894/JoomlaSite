...
if(($mergeMenuTreeMode = $this->sourceparams->get ( 'merge_menu_tree', null )) && !$isMindMap)  {
...
if (count ( $this->source->data ) != 0) {
	...
	foreach ( $this->source->data as $elm ) {
		...
		$category = ($isMindMap || !$linkableContentCats) ? $elm->category : '<a target="' . $openTarget . '" href="' . JRoute::_ ( ContentHelperRoute::getCategoryRoute($elm->catid ) ) . '">' . $elm->category . '</a>';
