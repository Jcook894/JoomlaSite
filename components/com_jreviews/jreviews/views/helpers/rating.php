...
//		$options[5] = 'Excellent';
    ...
    */
    function compareRatings($listing, $type)
    {
        ...
        {
            return '<div class="itemUserRating compareField">' . $this->drawStars($average_rating, $this->Config->rating_scale, $type) . '</div>';
        }
        
        // More than one criterion, display detailed ratings 
        $output = '<div class="itemUserRating compareField">' . $this->drawStars($average_rating, $this->Config->rating_scale, $type) . '</div>';
        
        $i = 0; 
        foreach($listing['Criteria']['criteria'] AS $key=>$value) 
        {
            $output .= '<div class="itemUserRating compareField' . (fmod($i, 2) ? '' : ' alt') . '">';
            $output .= $this->drawStars($ratings[$key], $this->Config->rating_scale, $type);
            $output .= '</div>';
            $i++; 
        }  
        return $output;                                                                    
