...
//            return $this->ajaxResponse($this->response);
    ...
    function _save()
    {
        ...
        ********************************************************************/
        ...
        if ($this->Access->loadWysiwygEditor())
        {
            $this->data['Listing']['introtext'] = html_entity_decode($this->data['__raw']['Listing']['introtext'],ENT_QUOTES,cmsFramework::getCharset());

            $this->data['Listing']['fulltext'] = html_entity_decode($this->data['__raw']['Listing']['fulltext'],ENT_QUOTES,cmsFramework::getCharset());
            ...
            if($this->Access->isEditor()) {
                ...
                $this->data['Listing']['introtext'] = Sanitize::stripWhitespace(Sanitize::getVar($this->data['Listing'],'introtext'));

                $this->data['Listing']['fulltext'] = Sanitize::stripWhitespace(Sanitize::getVar($this->data['Listing'],'fulltext'));
            }
            else {
        ...
        if ($isNew)
        ...
        else
            {
				...
				$this->data['Listing']['images']  = preg_replace('/{(.*)}/','',$this->data['Listing']['images']);
