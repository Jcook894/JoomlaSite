...
class JMapControllerMetainfo extends JMapController {
	...
	protected function setModelState($scope = 'default', $ordering = true) {
		...
		if(JRequest::getVar('metainfojsclient', false)) {
			$postedLang = JRequest::getVar('sitemaplang', null);
			if(!$postedLang) {
				$this->app->setUserState ( "$option.$scope.sitemaplang", null, '' );
			}
			$postedDataset = JRequest::getVar('sitemapdataset', null);
			if(!$postedDataset) {
				$this->app->setUserState ( "$option.$scope.sitemapdataset", null, '' );
			}
			$postedItemid = JRequest::getVar('sitemapitemid', null);
			if(!$postedItemid) {
