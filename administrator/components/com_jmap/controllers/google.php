...
class JMapControllerGoogle extends JMapController {
	...
	protected function setModelState($scope = 'default', $ordering = true) {
		...
		$defaultModel->setState ( 'googlestats', JRequest::getCmd('googlestats', 'analytics'));
	...
	public function display($cachable = false, $urlparams = false) {
		// PHP version check, 5.3+ required to use this feature
		if (version_compare(PHP_VERSION, '5.3.0', '<')) {
			$this->setRedirect('index.php?option=com_jmap&task=cpanel.display', JText::sprintf('COM_JMAP_PHP53_REQUIRED', PHP_VERSION));
			return false;
		}
	...
	public function deleteEntity() {
		...
		// Origin stats type
		$origin = JRequest::getVar('googlestats', null);
	...
	public function submitSitemap() {
		...
		$sitemapUri = JRequest::getString('sitemaplink');
		if(!$sitemapUri) {
	...
	public function deleteSitemap() {
		...
		// Retrieve the sitemap link
		$sitemapUri = JRequest::getString('sitemapurl');
	...
	public function markAsFixed() {
		...
		// Retrieve the sitemap link
		$crawlErrorsCategory = JRequest::getVar('crawlerrors_category');
