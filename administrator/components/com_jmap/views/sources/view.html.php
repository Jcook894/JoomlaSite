...
class JMapViewSources extends JMapView {
	...
	protected function addDisplayToolbar() {
		...
		if ($user->authorise('core.create', 'com_jmap')) {
			JToolBarHelper::custom('wizard.display', 'wizard', 'wizard', 'COM_JMAP_NEW_SOURCE', false);
		}
	
		if ($user->authorise('core.edit', 'com_jmap')) {
			JToolBarHelper::editListX('sources.editentity', 'COM_JMAP_EDIT_SOURCE');
		}
	
		JToolBarHelper::customX( 'sources.copyEntity', 'copy.png', 'copy_f2.png', 'COM_JMAP_DUPLICATE' );
	
		if ($user->authorise('core.delete', 'com_jmap') && $user->authorise('core.edit', 'com_jmap')) {
			JToolBarHelper::deleteListX('COM_JMAP_DELETE_SOURCE', 'sources.deleteentity');
		}
		
		if ($user->authorise('core.edit', 'com_jmap')) {
			JToolBarHelper::customX('sources.exportEntities', 'download', 'download', 'COM_JMAP_EXPORT_SOURCE', true);
			JToolBarHelper::customX('sources.importEntities', 'upload', 'upload', 'COM_JMAP_IMPORT_SOURCE', false);
		}
