...
class JMapViewGoogle extends JMapView {
	...
	protected function addDisplayToolbar() {
		...
		if($this->getModel()->getState('googlestats', 'analytics') == 'webmasters') {
			$doc->addStyleDeclaration('.icon-48-jmap{background-image:url("components/com_jmap/images/icon-48-googlewebmasters.png")}');
			JToolBarHelper::title( JText::_( 'COM_JMAP_GOOGLE_WEBMASTERS_TOOLS' ), 'jmap' );
		} else {
			$doc->addStyleDeclaration('.icon-48-jmap{background-image:url("components/com_jmap/images/icon-48-google.png")}');
		...
		if ($user->authorise('core.edit', 'com_jmap') && $this->getModel()->getState('googlestats', 'analytics') == 'webmasters' && $this->isLoggedIn) {
			JToolBarHelper::custom('google.submitSitemap', 'submit', 'submit', 'COM_JMAP_SUBMIT_SITEMAP', false);
		}
