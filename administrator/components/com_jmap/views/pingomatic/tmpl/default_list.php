...
<form action="index.php" method="post" name="adminForm" id="adminForm">
	<table class="full headerlist">
		<tr>
			<td align="left" width="100%">
				...
				<span class="input-group active">
				  <span class="input-group-addon"><span class="glyphicon glyphicon-th"></span> <?php echo JText::_('COM_JMAP_FILTER_BY_DATE_FROM' ); ?>:</span>
				  <input type="text" name="fromperiod" id="fromPeriod" data-role="calendar" size="60" value="<?php echo $this->dates['from'];?>" class="text_area"/>
				</span>
				<span class="input-group active">
				  <span class="input-group-addon"><span class="glyphicon glyphicon-th"></span> <?php echo JText::_('COM_JMAP_FILTER_BY_DATE_TO' ); ?>:</span>
				  <input type="text" name="toperiod" id="toPeriod" data-role="calendar" size="60" value="<?php echo $this->dates['to'];?>" class="text_area"/>
				</span>
				<button class="btn btn-primary btn-xs" onclick="this.form.submit();"><?php echo JText::_('COM_JMAP_GO' ); ?></button>
				...
				<a href="#pingomatic_stats" title="<?php echo JText::_('COM_JMAP_TITLE_PINGOMATIC_STATS');?>" class="fancybox btn btn-primary btn-xs absolute hasTooltip"><span class="glyphicon glyphicon-stats"></span> <?php echo JText::_('COM_JMAP_OPEN_PINGOMATIC_STATS' ); ?></a>
			</td>
		</tr>
	</table>
	...
	<thead>
		<tr>
			...
			<th nowrap="nowrap" class="title">
				<?php echo JHTML::_('grid.sort',  'COM_JMAP_LINKTITLE', 's.title', @$this->orders['order_Dir'], @$this->orders['order'] ); ?>
			</th>
			<th nowrap="nowrap" class="title">
				<?php echo JHTML::_('grid.sort',  'COM_JMAP_LINKURL', 's.blogurl', @$this->orders['order_Dir'], @$this->orders['order'] ); ?>
			</th>
			<th nowrap="nowrap" class="title">
				<?php echo JHTML::_('grid.sort',  'COM_JMAP_LINKRSS', 's.rssurl', @$this->orders['order_Dir'], @$this->orders['order'] ); ?>
			</th>
			<th nowrap="nowrap" class="title">
				<?php echo JHTML::_('grid.sort',  'COM_JMAP_LASTPING', 's.lastping', @$this->orders['order_Dir'], @$this->orders['order'] ); ?>
			</th>
			<th width="1%" nowrap="nowrap">
				<?php echo JHTML::_('grid.sort',   'COM_JMAP_ID', 's.id', @$this->orders['order_Dir'], @$this->orders['order'] ); ?>
			</th>
		</tr>
	</thead>
	<?php
	...
	for ($i=0, $n=count( $this->items ); $i < $n; $i++) {
		...
		if($this->user->authorise('core.edit', 'com_jmap')) {
			$checked = $row->checked_out && $row->checked_out != $this->user->id ? JHtml::_('jgrid.checkedout', $i, '', $row->checked_out_time, 'pingomatic.', false) : JHtml::_('grid.id', $i, $row->id);
		} else {
			$checked = null;
		}
		?>
...
<iframe class="hidden fancybox" id="pingomatic_stats"  width="660px" height="420px" src="index.php?option=com_jmap&amp;task=pingomatic.display&amp;format=raw"></iframe>