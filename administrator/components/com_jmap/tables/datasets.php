...
class TableDatasets extends JTable {
	...
	public function store($updateNulls = false) {
		...
		if($result) {
			...
			$queryDelete = "DELETE" .
						   "\n FROM " . $this->_db->nameQuote('#__jmap_dss_relations') .
						   "\n WHERE" .
						   "\n " . $this->_db->nameQuote('datasetid') . " = " .
						   "\n " . (int)$this->id;
			if(!$this->_db->setQuery($queryDelete)->query()) {
			...
			if(count($selectedSources)) {
				...
				$queryMultipleInsert = "INSERT" .
									   "\n INTO " . $this->_db->nameQuote('#__jmap_dss_relations') .
									   "\n (" . 
									   $this->_db->nameQuote('datasetid') . "," .
									   $this->_db->nameQuote('datasourceid') . ")" .
									   "\n VALUES " . $insertTuples;
				if(!$this->_db->setQuery($queryMultipleInsert)->query()) {
					$this->setError($this->_db->getErrorMsg());
					return false;
				}
			}
