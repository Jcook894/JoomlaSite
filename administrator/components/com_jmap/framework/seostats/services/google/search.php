...
class JMapSeostatsServicesGoogleSearch extends JMapSeostats {
	...
	protected static function makeRequest($pageNumber, $query, $result, $customHeaders, $onlyIndexedCount = false) {
		$ref = self::getReference ( $pageNumber, $query );
		$pageSerp = self::getPageSerp ( $pageNumber, $query );
		
		$curledSerp = self::gCurl ( $pageSerp, $ref, $customHeaders );
		
		// Get total number of indexed pages
		preg_match ( '#<div id="resultStats">(.*?)</div>#', $curledSerp, $matchesTotalIndexedPages );
		self::$numberIndexedPages = $matchesTotalIndexedPages;
		self::$paginationNumber = $pageNumber;
		if($onlyIndexedCount) {
		...
		// Parse SERP results
		self::parseResults ( $matches, $matchesDesc, $pageNumber * 10, $result );
	...
	protected static function gCurl($path, $ref, $customHeaders) {
		...
		if ($httpStatus >= 300 && $httpStatus < 400) {
			...
			if($redirectionURI) {
				return self::gCurl($redirectionURI, $ref, $customHeaders );
			}
		}
	...
	public static function getSerps($query, $pageNumber = 0, $customHeaders = array()) {
		...
		self::makeRequest ( $pageNumber / 10, $q, $result, $customHeaders);
