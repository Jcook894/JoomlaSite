...
class JMapFileSources extends JObject {
	...
	public function import() {
		...
		try {
			...
			$queryImport = 	"INSERT INTO" . $this->dbo->nameQuote('#__jmap') .
						   	"\n (" .
						   	"\n" . $this->dbo->nameQuote('type') . "," .
						   	"\n" . $this->dbo->nameQuote('name') . "," .
						   	"\n" . $this->dbo->nameQuote('description') . "," .
						   	"\n" . $this->dbo->nameQuote('published') . "," .
						   	"\n" . $this->dbo->nameQuote('sqlquery') . "," .
							"\n" . $this->dbo->nameQuote('sqlquery_managed') . "," .
							"\n" . $this->dbo->nameQuote('params') . ")" .
	...
	public function install() {
		// Get file info
		$file = JRequest::getVar('datasourceinstallplugin', array(), 'FILES');
		$tmpFile = $file['tmp_name'];
		$tmpFileName = $file['name'];
		try {
			if(!$tmpFile || !$tmpFileName) {
			...
			$_POST['name'] = ucfirst($pluginName);
			$_POST['description'] = (string)$pluginDescription;
			foreach ($pluginParams as $field) {
				...
				$_POST['params'][$fieldName] = $fieldValue;
			}
