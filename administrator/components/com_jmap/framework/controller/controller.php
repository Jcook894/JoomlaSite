 ...
 * @since 1.0
 */
class JMapController extends JController {
	/**
	...
	function getNames() {
		...
		if (empty ( $name )) {
			...
			if (! preg_match ( '/(.*)Controller(.*)/i', get_class ( $this ), $r )) {
				$this->app->enqueueMessage("JController::getName() : Cannot get or parse class name.", 'error');
			}
			$name = ($r [2]);
		}
	...
	function getView($name = null, $type = 'html', $prefix = null, $config = array()) {
		...
		if (empty ( $views [$name] )) {
			if ($view = $this->createView ( $name, $prefix, $type, $config )) {
				...
				$this->app->enqueueMessage(JText::_('View not found [name, type, prefix]:' ) . ' ' . $name . ',' . $type . ',' . $prefix, 'error');
				return false;
			}
		}
	...
	public function saveEntity() {
		...
		if (! $result = $model->storeEntity ()) {
			...
			$this->setRedirect ( "index.php?option=" . $this->option . "&task=" . $this->corename . ".editEntity&cid[]=" . JRequest::getVar('id'), JText::_ ( 'COM_JMAP_ERROR_SAVING' ) );
			return false;
		}
