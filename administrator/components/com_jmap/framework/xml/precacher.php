...
class JMapXmlPrecacher implements IJMapXmlPrecacher {
	 ...
	 * @return Object&
	 */
	public function __construct($fileName, JMapFileStream $fileWriter) {
		...
		// Set process status based on JS App Client - Require switcher for unit testing purpouse
		if(version_compare(JVERSION, '1.7', '<')) {
			$this->processStatus = JRequest::getCmd( 'process_status' );
		} else {
			$this->processStatus = JRequest::getVar( 'process_status' );
		}
