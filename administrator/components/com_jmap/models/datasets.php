...
class JMapModelDatasets extends JMapModel {
			...
			if(count($result)) {
				foreach ($result as &$row) {
					$subQuery = "SELECT" .
								...
								"\n WHERE " . $this->_db->nameQuote('id') . ' IN ( ' . preg_replace('/\[|\]/i', '', $row->sources) . ' )';
					$subQueryResults = $this->_db->setQuery($subQuery)->loadResultArray();
	...
	public function getLists($record = null) {
		...
		$query->from($this->_db->nameQuote('#__jmap'));
		$query->where($this->_db->nameQuote('published') . ' = 1');
