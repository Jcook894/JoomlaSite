...
class JMapModelSources extends JMapModel implements IJMapModelSources {
	...
	private function loadTableFields($record, $tableName) {
		...
		if(isset($record->sqlquery_managed->$tableName) && $record->sqlquery_managed->$tableName) {
			$queryFields = 	"SHOW COLUMNS " .
							"\n FROM " . $this->_db->nameQuote($record->sqlquery_managed->$tableName);
		...
		if(!empty($chunksObject->$fieldName)) {
			...
			foreach ($additionalParams as $param) {
				...
				if (count($subCkunks) > 1) {
					$firstSubChunk = $this->_db->nameQuote($chunksObject->$tableName) . "." . $this->_db->nameQuote($subCkunks[0]);
	...
	protected function buildRawQuery($chunksString, &$tableObject, $wizardClient = false, &$wizardModel = null) {
		...
		if(is_object($chunksObject)) {
			...
			$columnsQuery = "SHOW COLUMNS FROM " . $this->_db->nameQuote($chunksObject->table_maintable);
			...
			// Explicit required
			$asTitleField = !empty($chunksObject->titlefield_as) ? " AS " . $this->_db->nameQuote($chunksObject->titlefield_as) : null;
			$selectConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->titlefield) . $asTitleField;
			
			// Access field supported
			if(in_array('modified', $tableFields)) {
				$selectConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('modified');
			}

			// Explicit required
			if(!empty($tableFields) && isset($chunksObject->use_alias) && !!$chunksObject->use_alias && in_array('alias', $tableFields)) {
				$asIdField = !empty($chunksObject->idfield_as) ? " AS " . $this->_db->nameQuote($chunksObject->idfield_as) : " AS " . $this->_db->nameQuote($chunksObject->id);
				$selectConditions[] = "CONCAT_WS(':', " . 
									  $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->id) . ", " .
									  $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('alias'). ")" . $asIdField;
			} else {
				$asIdField = !empty($chunksObject->idfield_as) ? " AS " . $this->_db->nameQuote($chunksObject->idfield_as) : null;
				$selectConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->id) . $asIdField;
			}
			...
			if(!empty($chunksObject->catid)) {
				...
				if(isset($chunksObject->use_catalias) && 
					...
					$chunksObject->catid == $chunksObject->field_joinfrom_jointable1) {
						...
						$columnsQuery = "SHOW COLUMNS FROM " . $this->_db->nameQuote($chunksObject->table_joinwith_jointable1);
						$this->_db->setQuery($columnsQuery);
						if(!$joinTableFields = $this->_db->loadResultArray()) {
						...
						if(in_array('alias', $joinTableFields)) {
							$asCatidField = !empty($chunksObject->catidfield_as) ? " AS " . $this->_db->nameQuote($chunksObject->catidfield_as) : " AS " . $this->_db->nameQuote($chunksObject->catid);;
							$selectConditions[] = "CONCAT_WS(':', " . 
									  $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->catid) . ", " .
									  $this->_db->nameQuote($chunksObject->table_joinwith_jointable1) . "." . $this->_db->nameQuote('alias'). ")" . $asCatidField;
						}
				} else {
					$asCatidField = !empty($chunksObject->catidfield_as) ? " AS " . $this->_db->nameQuote($chunksObject->catidfield_as) : null;
					$selectConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->catid) . $asCatidField;
				}
			}
			// Implicit optional
			...
			for($jt=1,$maxJoin=3;$jt<=$maxJoin;$jt++) {
				...
				if(!empty($chunksObject->{'field_select_jointable'.$jt})) {
					$fieldAsJointable = !empty($chunksObject->{'field_as_jointable'.$jt}) ? " AS " . $this->_db->nameQuote($chunksObject->{'field_as_jointable'.$jt}) : null;
					$selectConditions[] = $this->_db->nameQuote($joinTable) . "." . $this->_db->nameQuote($chunksObject->{'field_select_jointable'.$jt}) . $fieldAsJointable;
				}
				// Implicit optional
				...
				for($wjt=1,$maxWhere=3;$wjt<=$maxWhere;$wjt++) {
					if(!empty($chunksObject->{'where'.$wjt.'_jointable'.$jt})) {
						...
						if(strpos($chunksObject->{'where'.$wjt.'_value_jointable'.$jt}, ',')) {
							...
							$whereConditions[] = $this->_db->nameQuote($joinTable) . "." . $this->_db->nameQuote($chunksObject->{'where'.$wjt.'_jointable'.$jt}) . $jtoperator . "(" . $chunksObject->{'where'.$wjt.'_value_jointable'.$jt} . ")";
						} else {
							$jtoperator = !empty($chunksObject->{'where'.$wjt.'_operator_jointable'.$jt}) ? " " . $chunksObject->{'where'.$wjt.'_operator_jointable'.$jt} . " " : " = ";
							$whereConditions[] = $this->_db->nameQuote($joinTable) . "." . $this->_db->nameQuote($chunksObject->{'where'.$wjt.'_jointable'.$jt}) . $jtoperator . $this->_db->quote($chunksObject->{'where'.$wjt.'_value_jointable'.$jt});
						}
					}
				}
				...
				$joinConditions[] = $joinType . " " . $this->_db->nameQuote($joinTable) . " ON " . 
									$this->_db->nameQuote($chunksObject->{'table_joinfrom_jointable'.$jt}) . "." . $this->_db->nameQuote($chunksObject->{'field_joinfrom_jointable'.$jt}) . " = " .
									$this->_db->nameQuote($chunksObject->{'table_joinwith_jointable'.$jt}) . "." . $this->_db->nameQuote($chunksObject->{'field_joinwith_jointable'.$jt});
				...
				if(!empty($chunksObject->{'orderby_jointable'.$jt})) {
					...
					array_unshift($orderbyConditions, $this->_db->nameQuote($chunksObject->{'table_joinwith_jointable'.$jt}) . "." . $this->_db->nameQuote($chunksObject->{'orderby_jointable'.$jt}) . $orderbyDirectionJointable);
				}
					
				//  ****GROUP BY FIELDS PROCESSING****
				if(!empty($chunksObject->{'groupby_jointable'.$jt})) {
					$groupbyConditions[] = $this->_db->nameQuote($joinTable) . "." . $this->_db->nameQuote($chunksObject->{'groupby_jointable'.$jt});
				}
			}
			...
			if(is_array($tableFields) && count($tableFields)) {
				...
				if(in_array('published', $tableFields)) {
					$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('published') . " = " . $this->_db->quote(1);
				} elseif(in_array('state', $tableFields)) { // State field supported fallback
					$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('state') . " = " . $this->_db->quote(1);
				}
			
				// Access field supported
				if(in_array('access', $tableFields)) {
					$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('access') . " IN {aid}" ;
				}
				...
				if($paramsRegistry->get('created_date', null) && in_array('created', $tableFields)) {
					...
					$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('created') . " > {" . $latestMonths . "months}" ;
				}
				
				// Language field supported
				if(in_array('language', $tableFields)) {
					$whereConditions[] =  " (" . $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('language') . " = " . $this->_db->quote('*') . 
										 " OR " . $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote('language')  . " = {langtag})";
				}
			}
			// *EXPLICIT WHERE PART*
			for($wmt=1,$maxWhere=3;$wmt<=$maxWhere;$wmt++) {
				if(!empty($chunksObject->{'where'.$wmt.'_maintable'})) {
					...
					if(strpos($chunksObject->{'where'.$wmt.'_value_maintable'}, ',')) {
						...
						$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->{'where'.$wmt.'_maintable'}) . $operator . "(" . $chunksObject->{'where'.$wmt.'_value_maintable'} . ")";
					} else {
						$operator = !empty($chunksObject->{'where'.$wmt.'_operator_maintable'}) ? " " . $chunksObject->{'where'.$wmt.'_operator_maintable'} . " " : " = ";
						$whereConditions[] = $this->_db->nameQuote($mainTable) . "." . $this->_db->nameQuote($chunksObject->{'where'.$wmt.'_maintable'}) . $operator . $this->_db->quote($chunksObject->{'where'.$wmt.'_value_maintable'});
					}
				}
			}
			...
			if(!empty($chunksObject->{'orderby_maintable'})) {
				...
				array_push($orderbyConditions, $this->_db->nameQuote($chunksObject->{'table_maintable'}) . "." . $this->_db->nameQuote($chunksObject->{'orderby_maintable'}) . $orderbyDirectionMaintable);
			}	
			
			//  ****GROUP BY FIELDS PROCESSING****
			if(!empty($chunksObject->{'groupby_maintable'})) {
				$groupbyConditions[] = $this->_db->nameQuote($chunksObject->{'table_maintable'}) . "." . $this->_db->nameQuote($chunksObject->{'groupby_maintable'});
			}
			...
			$finalQueryString .= "\n FROM " . $this->_db->nameQuote($chunksObject->table_maintable);
	...
	public function getLists($record = null) {
		...
		$queryComponent = "SELECT DISTINCT " . $this->_db->nameQuote('element') . " AS value, SUBSTRING(" . $this->_db->nameQuote('element') . ", 5) AS text" .
						  ...
						  "\n OR " . $this->_db->nameQuote('element') . " = " . $this->_db->quote('com_content') . // Exception for custom content sources
						  "\n OR " . $this->_db->nameQuote('element') . " = " . $this->_db->quote('com_tags') . // Exception for custom tags sources
						  "\n OR " . $this->_db->nameQuote('element') . " = " . $this->_db->quote('com_docman') . ")" . // Exception for docman 2 protected
	...
	public function getHasCreatedDate($record) {
		...
		// Build query
		$query = "SHOW COLUMNS FROM " . $this->_db->nameQuote($record->sqlquery_managed->table_maintable);
		$this->_db->setQuery ( $query );

		try {
			$tableFields = $this->_db->loadResultArray ();
			if ($this->_db->getErrorNum ()) {
				...
				try {
					...
					if($table->type === 'plugin') {
						$otherResourcesQuery = "SELECT COUNT(*)" .
											   "\n FROM " . $this->_db->nameQuote('#__jmap') .
											   "\n WHERE" .
											   "\n " . $this->_db->nameQuote('type') . " = " . $this->_db->quote('plugin') .
											   "\n AND " . $this->_db->nameQuote('name') . " = " . $this->_db->quote($table->name) .
											   "\n AND " . $this->_db->nameQuote('id') . " != " . (int)$table->id;
