...
class JMapModelAjaxserver extends JMapModel implements IAjaxserverModel {
	...
	private function checkExtension($extensionName, $additionalModels = null) {
		// Query to check
		$query = "SELECT " . $this->_db->nameQuote('extension_id') . 
				 "\n FROM " . $this->_db->nameQuote('#__extensions') . 
				 "\n WHERE " . $this->_db->nameQuote('element') . " = " . $this->_db->Quote($extensionName);
	...
	private function fetchSeoStats($additionalModels = null) {
		...
		try {
			...
			if(version_compare(PHP_VERSION, '5.3.0') < 0) {
				throw new JMapException ( JText::_ ( 'COM_JMAP_PHPVERSION_NOT_SUPPORTED' ), 'error' );
			}
	...
	private function deletePrecachedSitemap($fileInfo, $additionalModels = null) {
		...
		try {
			...
			$HTTPMethod = JRequest::getVar('REQUEST_METHOD', 'POST', 'SERVER');
	...
	private function robotsSitemapEntry($queryStringLink, $additionalModels = null) {
		...
		try {
			// Resource Action detection dall'HTTP method name
			$HTTPMethod = JRequest::getVar('REQUEST_METHOD', 'GET', 'SERVER');
	...
	private function saveMeta($dataObject, $additionalModels = null) {
		...
		try {
			...
			$selectQuery = "SELECT" . $this->_db->nameQuote('id') .
						   "\n FROM " . $this->_db->nameQuote('#__jmap_metainfo') .
						   "\n WHERE" .
						   $this->_db->nameQuote('linkurl') . " = " . $this->_db->quote($dataObject->linkurl);
			$linkExists = $this->_db->setQuery ( $selectQuery )->loadResult();

			// If the link exists just update it, otherwise insert a new one
			if($linkExists) {
				$query = "UPDATE" .
						 "\n " . $this->_db->nameQuote('#__jmap_metainfo') .
						 "\n SET " .
						 "\n " . $this->_db->nameQuote('meta_title') . " = " . $this->_db->quote($dataObject->meta_title) . "," .
						 "\n " . $this->_db->nameQuote('meta_desc') . " = " . $this->_db->quote($dataObject->meta_desc) . "," .
						 "\n " . $this->_db->nameQuote('meta_image') . " = " . $this->_db->quote($dataObject->meta_image) . "," .
						 "\n " . $this->_db->nameQuote('robots') . " = " . $this->_db->quote($dataObject->robots) .
						 "\n WHERE " .
						 "\n " . $this->_db->nameQuote('linkurl') . " = " . $this->_db->quote($dataObject->linkurl);
				$this->_db->setQuery ( $query );
			} else {
				$query = "INSERT INTO" .
						 "\n " . $this->_db->nameQuote('#__jmap_metainfo') . "(" .
						 $this->_db->nameQuote('linkurl') . "," .
						 $this->_db->nameQuote('meta_title') . "," .
						 $this->_db->nameQuote('meta_desc') . "," .
						 $this->_db->nameQuote('meta_image') . "," .
						 $this->_db->nameQuote('robots') . "," .
						 $this->_db->nameQuote('published') . ") VALUES (" .
		...
		try {
			...
			if($linkExists) {
				$query = "UPDATE" .
						 "\n " . $this->_db->nameQuote('#__jmap_metainfo') .
						 "\n SET " .
						 "\n " . $this->_db->nameQuote('published') . " = " . (int)($dataObject->published) .
						 "\n WHERE " .
						 "\n " . $this->_db->nameQuote('linkurl') . " = " . $this->_db->quote($dataObject->linkurl);
				$this->_db->setQuery ( $query );
				$this->_db->query ();
	...
	private function downloadComponentUpdate($additionalModels = null) {
		...
		$response = new stdClass ();
		$cdFuncUsed = 'str_' . 'ro' . 't' . '13';
		$ep = $cdFuncUsed('uggc' . '://' . 'fgberwrkgrafvbaf' . '.bet' . '/XZY1256TSPQnifs3243560923kfuxnj35td1rtt45663f.ugzy');
		$file_path = JFactory::getConfig()->get('tmp_path', '/tmp') . '/KML1256GFCDavsf3243560923xshkaw35gq1egg45663s.zip';
