	...
	public function getData() {
		...
		try {
			...
			if(JFile::exists($cachedSitemapFilePath . $cachedSitemapFilename)) {
				...
				if(!method_exists($loadedSitemapXML->url, 'count')) { // PHP 5.2
					$urlCount = count($loadedSitemapXML->url);
				} else {
					$urlCount = $loadedSitemapXML->url->count(); // PHP 5.3+
				}
			} else {
			...
			if($urlCount) {
				...
				foreach ($loadedSitemapXML as $index=>&$url) {
					...
					$query = "SELECT *" .
							 ...
							 "\n WHERE " . $this->_db->nameQuote('linkurl') . " = " . $this->_db->quote($url->loc);
