...
class JMapModelGoogle extends JMapModel implements IJMapModelGoogle {
	...
	private function authentication($params) {
		...
		if (! $result = $this->client->getAccessToken ()) { // auth call to google
			...
			if (!JRequest::getVar('ga_dash_authorize')) {
				$JText = 'JText';
				$htmlSnippet = <<<HTML
					<div>
						<p class="well">
...
HTML;
				...
				try {
					$this->client->authenticate ( JRequest::getVar('ga_dash_code'));
	...
	public function getData() {
		...
		if (JRequest::getVar('gaquery')) {
			$gaquery = JRequest::getVar('gaquery');
		} else {
			$gaquery = "visits";
		}
		
		if (JRequest::getVar('gaperiod')) {
			$gaperiod = JRequest::getVar('gaperiod');
		} else {
			$gaperiod = "last30days";
		}
