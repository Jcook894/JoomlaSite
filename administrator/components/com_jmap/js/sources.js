...
jQuery(function($){
	var Sources = $.newClass ({
		...
		registerEvents : function() {
			...
			$('#controls_merge_menu_tree label').on('click', function(jqEvent){
				if(jqEvent.target.nodeName.toLowerCase() == 'label') {
					var labelId = $(this).attr('id');
					if(labelId.match(/yes/i)) {
						$('#params\\[multilevel_categories\\]1-lbl').trigger('click');
					}
				}
			});
			
			// Change the label accordingly based on the exclude/include switcher
			$('#paramschoose_exclusion_way_include-lbl, #paramschoose_exclusion_way_exclude-lbl').on('click', function(jqEvent){
				var selectedExclusionWay = $(this).prev().val();
				...
				switch (selectedExclusionWay) {
					case 'exclude':
						...
						$('#paramschoose_catexclusion-lbl').data('bs.popover').options.content = COM_JMAP_CHOOSE_CATEGORIES_EXCLUSION_DESC;
						$('#paramschoose_catexclusion-lbl').attr('data-content', COM_JMAP_CHOOSE_CATEGORIES_EXCLUSION_DESC);
						$('#paramschoose_artexclusion-lbl').text(COM_JMAP_CHOOSE_ARTICLES_EXCLUSION);
						$('#paramschoose_artexclusion-lbl').data('bs.popover').options.content = COM_JMAP_CHOOSE_ARTICLES_EXCLUSION_DESC;
						$('#paramschoose_artexclusion-lbl').attr('data-content', COM_JMAP_CHOOSE_ARTICLES_EXCLUSION_DESC);
					...
					case 'include':
						...
						$('#paramschoose_catexclusion-lbl').data('bs.popover').options.content = COM_JMAP_CHOOSE_CATEGORIES_INCLUSION_DESC;
						$('#paramschoose_catexclusion-lbl').attr('data-content', COM_JMAP_CHOOSE_CATEGORIES_INCLUSION_DESC);
						$('#paramschoose_artexclusion-lbl').text(COM_JMAP_CHOOSE_ARTICLES_INCLUSION);
						$('#paramschoose_artexclusion-lbl').data('bs.popover').options.content = COM_JMAP_CHOOSE_ARTICLES_INCLUSION_DESC;
						$('#paramschoose_artexclusion-lbl').attr('data-content', COM_JMAP_CHOOSE_ARTICLES_INCLUSION_DESC);
					break;
				}
			});
