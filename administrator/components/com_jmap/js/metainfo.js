...
(function($) {
	var Metainfo = function() {
		...
		var addListeners = function(initialize) {
			...
			$('#adminForm div.pagination a').on('click.metainfo', function(jqEvent){
				// Show striped progress started generation
			...
			$('#adminForm button[data-action=savemeta]').on('click.metainfo', function(jqEvent) {
				...
				var metaImage = $('input[data-mediaidentifier=' + rowIdentifier + ']').val();
			...
			$('#adminForm div[data-action=statemeta] label').on('click.metainfo', function(jqEvent, noTrigger) {
				...
				// Retrive information to save
				var rowIdentifier = $(this).parents('div').data('state');
				var linkIdentifier = $('a[data-linkidentifier=' + rowIdentifier + ']').attr('href');
				var publishedState = $(this).prev('input').val();
		...
		var rowButtonStatus = function(specificRow) {
			...
			$(finalSelector).each(function(index, tableRow){
				...
				if(!titleValue && !descValue && !imageValue && !robotsValue) {
					$('button, div.controls label', tableRow).attr('disabled', true);
					$('div.controls label', tableRow).addClass('jmap_inactive');
				} else {
					$('button, div.controls label', tableRow).removeAttr('disabled');
					$('div.controls label', tableRow).removeClass('jmap_inactive');
				}
			});
		};
