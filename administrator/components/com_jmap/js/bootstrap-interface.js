...
jQuery(function($) {
	 ...
	 */
	$('label.hasTooltip, img.hasTooltip, a.hasTooltip, span.hasTooltip, span.hasTip, a.hasTip').tooltip({trigger:'hover', placement:'top'});
	...
	$('div.panel-group').on('shown.bs.collapse', function (event) {
		...
		if(document.body.scrollHeight > window.innerHeight && $(this).attr('id') != 'accordion_cpanel') {
			$('html, body').animate({ scrollTop: $("#"+event.target.id).prev().offset().top - 100 }, 500);
		}
	...
	if(sitemapTemplate != 'mindmap') {
		$('*.mindmap_styles').parent('div.paramcontainer').hide();
		$('span.mindmap_styles').parents('div.paramcontainer').hide();
	}
	$('select[name=params\\[sitemap_html_template\\]]').on('change', function(){
		if($(this).val() == 'mindmap') {
			$('*.mindmap_styles').parent('div.paramcontainer').show();
			$('span.mindmap_styles').parents('div.paramcontainer').show();
		} else {
			$('*.mindmap_styles').parent('div.paramcontainer').hide();
			$('span.mindmap_styles').parents('div.paramcontainer').hide();
		}
