...
jQuery(function($){
	var CPanel = $.newClass ({
    	...
    	updateButtonSnippet : '<button id="updatebtn" data-content="' + COM_JMAP_EXPIREON + '%EXPIREON%" class="btn btn-small btn-primary">' + 
    							'<span class="glyphicon glyphicon-upload"></span> ' + COM_JMAP_CLICKTOUPDATE + 
    						  '</button>',
		...
		registerEvents : function() {
			...
			$('label.hasClickPopover, input.hasClickPopover, li.hasClickPopover').on('hidden.bs.popover', function(event){
				var nextPopoverIfAny = $(event.target).next('div.popover.left');
				if(nextPopoverIfAny.length) {
					nextPopoverIfAny.remove();
				}
			});
        ...
        addRobotsEntry : function() {
			...
			var messageSnippet =    '<dl class="robots_messages" id="system-message">' +
										'<dt class="message">Message</dt>' +
										'<dd class="message message">' +
											'<ul><li>' + COM_JMAP_ROBOTS_ENTRY_ADDED + '</li></ul>' +
										'</dd>' +
									'</dl>';
        	// Retrieve values
