...
class plgSystemJMap extends JPlugin {
	...
	public function onAfterInitialise() {
		...
		if( JRequest::getVar ( 'option' ) == 'com_jmap' && $this->jmapConfig->get('advanced_multilanguage', 0)) {
			...
			if (isset($sefs[$lang])) {
				...
				JRequest::setVar(JApplication::getHash('language'), $lang_code, 'cookie');
				...
				if(!$removeDefaultPrefix || ($removeDefaultPrefix && $defaultSiteLanguage != $lang_code)) {
					// Check if the default language is not the current one
		...
		if ($this->isBotRequest () && $option == 'com_jmap') {
			...
			JRequest::setVar ( 'nobotsef', true, 'POST' );
		}
	}
	...
	public function onAfterRoute() {
		...
		if (! $this->app->getClientId () || version_compare ( JVERSION, '2.5', '<' )) {
			return;
		}
	...
	public function onBeforeCompileHead() {
		...
		$query = "SELECT *" .
				 ...
				 "\n WHERE " . $db->nameQuote('linkurl') . " = " . $db->quote($uri) .
		...
		if(isset($metaInfoForThisUri->id)) {
			...
			if($title) {
				// Append site name
				if ($app->getCfg('sitename_pagetitles', 0) == 2 && trim($app->getCfg('sitename'))) {
					$title = $title . ' - ' . trim($app->getCfg('sitename'));
				} elseif ($app->getCfg('sitename_pagetitles', 0) == 1 && trim($app->getCfg('sitename'))) { // Prepend site name
					$title = trim($app->getCfg('sitename')) . ' - ' . $title;
				}
		...
		$isPagination = JRequest::getVar('start', null, 'GET', 'INT');
		$isPage = JRequest::getVar('page', null, 'GET', 'INT');
	...
	public function onRenderModule() {
		...
		// Ensure that the JDocumentError class is instantiated as singleton from the legacy JError class and an error is present
	...
	public function onAfterRender() {
		...
		if ( $option == 'com_jmap' && JRequest::getVar('format') ) {
			return false;
		}
...
JS;
	...
	public function __construct(&$subject) {
		...
		$app = JFactory::getApplication ();
		if((JRequest::getVar('option') == 'com_modules' || JRequest::getVar('option') == 'com_advancedmodules') &&
			JRequest::getVar('view') == 'module' &&
